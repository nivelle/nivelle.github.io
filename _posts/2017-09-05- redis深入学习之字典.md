...
- 当以下条件满足任意一个时，程序就会对哈希表进行扩展操作
 ...
 1. 服务器目前没有执行bgsave或bgrewriteaof命令，并且哈希表的负载因子>=1
 2. 服务器目前正在执行bgsave或bgrewriteaof命令，并且哈希表的负载因子>=5
...
- 为ht[1]哈希表分配空间,这个哈希表的空间大小取决于要执行的操作,以及ht[0]当前包含的键值对数量(也即ht[0].used)
  ...
  - 如果执行的是拓展操作,那么ht[1]的大小为第一个大于等于ht[0].used*2的2^n(2的n次方幂);例如 ht[0].used的值为4，而4*2=8，大于等于它的2^n是8，所以将ht[1]的大小设置为8
...
![](https://ws1.sinaimg.cn/large/b1eb59d9ly1fxrobd491yj20t20kb3z6.jpg)

- rehash索引0上的键值对

![](https://ws1.sinaimg.cn/large/b1eb59d9ly1fxrodgxmdoj20ud0jgglx.jpg)

- rehash索引1上的键值对

![](https://ws1.sinaimg.cn/large/b1eb59d9ly1fxroelvansj20tp0jdgm9.jpg)

- rehash索引2上的键值对

![](https://ws1.sinaimg.cn/large/b1eb59d9ly1fxroffv1ptj20t40iw0td.jpg)

- rehash索引3上的键值对

![](https://ws1.sinaimg.cn/large/b1eb59d9ly1fxrogfwc01j20u20k7q3m.jpg)

- 渐进式rehash完成

![](https://ws1.sinaimg.cn/large/b1eb59d9ly1fxroh3mkj9j20up0frq3h.jpg)
